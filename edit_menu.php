...
$user = $conn->query("SELECT * FROM dishes d join restaurant r on r.rs_id=d.rs_id WHERE d_id = $id")->fetch_assoc();
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($errors)) {
        $sql = "UPDATE dishes SET 
                ...
                describtion='$about',
                price='$price',    
                img='$image_saved_path'
            WHERE d_id = $id";

        $conn->query($sql);
        $sql = "Update restaurant set c_id='$category' where rs_id =(select rs_id from dishes where d_id=$id)";
        $conn->query($sql);
...
<body>
    ...
    <div class="main d-flex">
        ...
        <div class="total flex-grow-1">
            <div class="Title">Add Menu to Restaurant</div>
            ...
            <form class="row g-3" method="POST" enctype="multipart/form-data">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control" id="inputabout" name="about" value="<?= $user['describtion']   ?>">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control" id="inputPrice" name="price" value="<?= $user['price']  ?>">
                    ...
                    <select name="category" class="form-select <?= isset($errors['category']) ? 'is-invalid' : '' ?>">
                        ...
                        if (mysqli_num_rows($result) > 0) {
                            ...
                            while ($row = mysqli_fetch_assoc($result)) {
                                $sel = ($user['c_id'] == $row['c_id']) ? 'selected' : '';
